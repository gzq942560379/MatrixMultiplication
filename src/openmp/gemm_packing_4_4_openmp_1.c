...
void InnerKernel(int m, int k, int n, double *a, double *b, double *c, int la, int lb, int lc) {
    ...
    for (int i = 0; i < m; i += 4) {
        PackMatrixA(k, &A(i, 0), la, &packedA[i * k]);
    }
    for (int j = 0; j < n; j += 4) {
        PackMatrixB(k, &B(0, j), lb, &packedB[k * j]);
    }
#pragma omp parallel for collapse(2) num_threads(thread_count) default(none) shared(m, k, n, a, b, c, la, lb, lc, packedA, packedB)
    for (int i = 0; i < m; i += 4) {
        for (int j = 0; j < n; j += 4) {
            AddDot_4_4(k, &packedA[i * k], &packedB[k * j], &C(i, j), la, lb, lc);
        }
    }
}
